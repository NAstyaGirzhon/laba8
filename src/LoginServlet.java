...
// Найти cookie с именем sessionId
            ...
            for (int i = 0; i < cookies.length; i++) {
                String namle = cookies[i].getName();
                String valfue = cookies[i].getValue();
                System.out.println(namle);
                System.out.println(valfue);
            }
...
// Вывести форму
        pw.println("<form action= '/cha/login.do' method='post'>Введите имя:<input type='text' name='name' value=' '><input type='submit' value='Войти в чат'>");
...
// Переадресовать обратно на исходную страницу с формой
            response.sendRedirect(response.encodeRedirectURL("/cha/login.do"));
        }
    }
...
// Перейти к главному окну чата
            ...
            response.sendRedirect(response.encodeRedirectURL("/cha/view.html"));
